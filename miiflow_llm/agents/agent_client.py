...
class AgentClient:
    ...
    async def run(
        ...
        **kwargs,
        ...
        logger.debug(
            f"Final metrics - Individual tool calls: {tool_calls_count}, Tool result messages: {tool_messages_count}"
        )
    ...
    async def stream_react(
        ...
        **kwargs,
        ...
        run_context = RunContext(
            deps=context.deps if hasattr(context, "deps") else None, messages=message_history or []
        )
    ...
    async def stream_single_hop(self, user_prompt: str, *, context: RunContext):
        """Stream single-hop execution - matches core agent signature exactly."""
        try:
            async for event in self.agent.stream_single_hop(user_prompt, context=context):
                yield event
        except Exception as e:
            # Emit error event with AgentClient context
            yield {
...
def create_agent(config: AgentConfig) -> AgentClient:
    ...
    try:
        llm_client = LLMClient.create(
            ...
            model=config.model,
            api_key=config.api_key,  # Pass API key from config
        )
    except Exception as e:
